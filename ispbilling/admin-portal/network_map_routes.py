...
def api_items(request: Request):
    ...
    with engine.begin() as conn:
        ...
        for r in rows:
            ...
            # _S46A_  Skip OLTs with no/invalid coordinates (incl. Null
            # Island 0,0). Admin must manually set lat/lng to pin them.
            if r[4] is None or r[5] is None: continue
            try:
                if abs(float(r[4])) < 0.0001 and abs(float(r[5])) < 0.0001:
                    continue
            except Exception:
                continue
...
def api_pon_status(request: Request, olt_id: int):
    ...
    with engine.begin() as conn:
        ...
        # _S47C_  also pull real per-PON temperature/voltage/bias
        rows = conn.exec_driver_sql(
        ...
        for r in rows:
            ...
            out.append({
                ...
                # _S47C_  r[2] is real OLT-side TX (from `show pon optical
                # transceiver`). DO NOT fall back to ONU-side max — that
                # mixes physical layers and produced bogus +2 dBm.
                "tx_power": r[2],
                "admin_up": bool(r[3]),
                "oper_up": bool(r[4]),
                ...
                "temperature_c": (r[7] if len(r) > 7 else None),
                "voltage": (r[8] if len(r) > 8 else None),
                "bias_curr_ma": (r[9] if len(r) > 9 else None),
            })
