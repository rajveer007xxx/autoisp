                ...
                if env.get("ok"):
                    for k in ("cpu_pct", "temp_c", "voltage_v", "fan_status",
                        ...
                        if env.get(k) is not None:
                            base.olt[k if k != "uptime_s" else "uptime_sec"] = env[k]
            return base
    except Exception:
        pass
...
def page_olt_detail_admin(olt_id: int, request: Request):
    """__S44U__ Per-OLT detail dashboard (iFastX-parity layout)."""
    sc = _require_scope(request)
    cid = sc["company_id"]
    with engine.begin() as conn:
        ...
        try:
            recent = conn.exec_driver_sql(
                "SELECT serial, status, pon_port_index, onu_index, rx_power, "
        ...
        try:
            alerts = conn.exec_driver_sql(
                "SELECT level, title, created_at FROM olt_alerts "
                "WHERE olt_id=? AND COALESCE(resolved,0)=0 "
                "ORDER BY created_at DESC LIMIT 10", (olt_id,)).fetchall()
        except Exception:
            alerts = []
    ...
    for i in range(1, pon_count + 1):
        ...
        avg_tx = (sum(d["tx"])/len(d["tx"])) if d["tx"] else None
        # Status logic
        if not d["rx"]:
            status = "idle"
        elif avg_rx <= -28:
    ...
    # _S45F_REAL_UPLINKS — query ifAdminStatus/ifOperStatus over SNMP for
    # the first uplink_count non-PON interfaces. Falls back to "unknown"
    # when SNMP is unavailable so we never fabricate "up".
    uplink_ports = []
    uplink_real = []
    try:
        from olt_vendors import _snmp_walk
        comm = olt[14] if len(olt) > 14 and olt[14] else "public"
        # Best-effort: walk ifDescr + ifOperStatus on standard MIB.
        desc = dict(_snmp_walk(olt[3], comm, 161,
                                "1.3.6.1.2.1.2.2.1.2", "v2c", timeout=2.0))
        oper = dict(_snmp_walk(olt[3], comm, 161,
                                "1.3.6.1.2.1.2.2.1.8", "v2c", timeout=2.0))
        # Collect non-PON, non-loopback ifaces (label heuristic)
        candidates = []
        for ifidx, label in desc.items():
            ...
            if any(t in lab_low for t in ("loopback", "null", "console")):
                continue
            st = oper.get(ifidx, "0")
            # ifOperStatus: 1=up, 2=down, others=intermediate
            uplink_real.append((label, "up" if st.strip() == "1"
                                  else "down" if st.strip() == "2"
                                  else "unknown"))
            if len(uplink_real) >= uplink_count:
                break
    except Exception:
        uplink_real = []

    if uplink_real:
        for i, (label, st) in enumerate(uplink_real, 1):
            uplink_ports.append({
                "index": i, "label": label or f"GE0/{i}",
                "status": st, "speed": "1G",
            })
    else:
        # SNMP not reachable for ifTable — show declared port count with
        # explicit "unknown" status (NOT "up"), so admin sees data gap.
        for i in range(1, uplink_count + 1):
            uplink_ports.append({
                "index": i, "label": f"GE0/{i}",
                "status": "unknown", "speed": "1G",
            })
    ...
    ctx["alerts"] = [
        {"level": a[0], "title": a[1], "when": a[2]} for a in alerts
    ]
    ...
    with engine.begin() as conn:
        ...
        conn.exec_driver_sql(
            ...
            (body.name, body.vendor, body.model or "", body.host,
             ...
             body.whatsapp_instance_id or "", wa_key_enc,
             vpn_type_lc, body.vpn_username or "", vpn_pw_enc,
             body.vpn_endpoint or "", olt_id, cid),
        )
    return {"ok": True}
...
def api_list_onus(
    ...
    sql = ("SELECT n.id, n.olt_id, o.name AS olt_name, n.pon_port_index, "
           ...
           # _S46A_ Fiber link + TR069 columns
    ...
    for r in rows:
        ...
        # to GenieACS (proxy: wifi_ssid is non-empty since we only push
...
def api_patch_onu(request: Request, onu_id: int, body: OnuPatch):
    ...
    if _wifi_or_link_changed:
        auto = _genieacs_auto_push(cid, onu_id, reason="onu patch")
...
def api_onu_wifi(request: Request, onu_id: int, body: WifiIn):
    ...
    auto-push to GenieACS. Backwards-compatible: callers passing only
    ...
    # _S40_AUTOPROV_ — push to TR-069 immediately
    auto = _genieacs_auto_push(cid, onu_id, reason=f"wifi by {actor}")
...
def api_onu_wan(request: Request, onu_id: int, body: WanIn):
    ...
    # _S40_AUTOPROV_ — push to TR-069 immediately
    auto = _genieacs_auto_push(cid, onu_id, reason=f"wan/{body.wan_mode} by {actor}")
...
def _genieacs_provision(cid: str, *, device_serial: str,
    ...
    """POST a SetParameterValues task to GenieACS. Returns result dict.
       _S40zd_  Now resolves the real GenieACS device _id by querying
    ...
    try:
        ...
        device_id = None
        try:
            q = _j.dumps({"_deviceId._SerialNumber": device_serial})
            qurl = (base + "/devices?query=" + urllib.parse.quote(q)
                    + "&projection=_id")
        ...
        if not device_id:
            return {"ok": False,
                    "error": (f"ONU '{device_serial}' is not registered "
                              "with GenieACS yet. The CPE must connect at "
...
def _genieacs_auto_push(cid: str, onu_id: int, *, reason: str = "") -> dict:
    ...
    ONU + linked customer state, then push to GenieACS. Used by the
    auto-bridge after wifi/wan/customer changes. Always non-throwing —
    returns {"ok": True/False, ...}.

    Skips silently when:
      * GenieACS URL is not configured for the company
    ...
    try:
        ...
        # If linked to a customer and that customer has pppoe creds set,
        # prefer them (single source of truth — billing system).
        if cust_id:
            try:
                with engine.begin() as conn:
                ...
                if crow:
                    if crow[0]:
                        wan_user = wan_user or crow[0]
                    if crow[1]:
                        wan_pw = wan_pw or crow[1]
                    if not wan_mode:
                        wan_mode = "pppoe"
            except Exception:
                pass
...
# ──────────────────────────────────────────────────────────────────────────
# _S39R5Q — TR-069 / GenieACS admin web UI
# ──────────────────────────────────────────────────────────────────────────
...
def api_genie_recent_pushes(request: Request, limit: int = 10):
    ...
    GenieACS page widget). Most recent first."""
    cid, _ = _require_admin(request)
...
def api_delete_onu(request: Request, onu_id: int):
    sc = _require_scope(request); cid, actor = sc["company_id"], sc["actor"]
    with engine.begin() as conn:
        row = conn.exec_driver_sql(
        ...
        if sc["role"] == "sub_lco":
            ...
            if row[6] != int(eid):
                ...
                if not (er and er[0] and row[5] == int(er[0])):
                    raise HTTPException(403, "ONU not in your employee scope")
        # Safety guard — ONU must be offline / unprovisioned. Refuse to
        # delete a live customer's ONU; the operator must factory-reset
        # (or terminate the customer) first.
        status = (row[3] or "").lower()
        if status == "online":
            raise HTTPException(400, "Cannot delete an ONLINE ONU. "
                "Factory-reset or unprovision it first.")
        conn.exec_driver_sql(
        ...
        try:
            conn.exec_driver_sql(
                ...
                (cid, actor, "delete", "onu", onu_id,
                 f"serial={row[2]} olt_id={row[1]}"))
        except Exception:
            pass
...
def api_onu_bulk_repush(request: Request, body: BulkRepushIn):
    ...
    s = _settings_for(cid)
    if not (s.get("genieacs_url") or "").strip():
        raise HTTPException(400, "GenieACS URL not configured")
    where = ["company_id = ?"]
    args = [cid]
    if body.onu_ids:
...
def api_olt_vpn_provision(request: Request, olt_id: int):
    ...
    if vpn_type not in ("openvpn", "l2tp_ipsec", "pptp"):
        raise HTTPException(400, f"Use the WireGuard 'VPN' button for vpn_type={vpn_type}")
    if vpn_type == "openvpn":
...
# ──────────────────────────────────────────────────────────────────────────
@router.get("/api/admin/olt/olts/{olt_id}/vpn-status")
def api_olt_vpn_status(request: Request, olt_id: int):
    """Returns live connection state for the OLT's configured non-WG VPN.
    Parses the daemon's status log / runtime peer table."""
    cid, _ = _require_admin(request)
    ...
    if vpn_type == "none":
        return {"ok": True, "status": "n/a",
                "message": "OLT is not configured for a VPN."}
    if vpn_type == "wireguard":
        return {"ok": True, "status": "redirect",
                "message": "Use /wg-status for WireGuard OLTs."}
    if not user:
        # _S45F_SHARED_TUNNEL — when this OLT has no cert of its own, look
        # for a sibling OLT in the same tenant whose tunnel is currently
        # up AND whose iroute covers this OLT's host. Common case: 2 OLTs
        # on the same customer LAN behind one Mikrotik share ONE tunnel.
        import ipaddress as _ipa, os as _os
        with engine.begin() as _conn:
            _myhost = _conn.exec_driver_sql(
                "SELECT host FROM olts WHERE id=?", (olt_id,)).fetchone()
            _siblings = _conn.exec_driver_sql(
                "SELECT id, name, vpn_username FROM olts "
                "WHERE company_id=? AND id<>? AND IFNULL(vpn_username,'')<>''",
                (cid, olt_id)).fetchall()
        if _myhost and _siblings:
            try:
                _myip = _ipa.ip_address((_myhost[0] or "").strip())
            except Exception:
                _myip = None
            try:
                _log = open("/var/log/openvpn/olt-status.log").read()
            except Exception:
                _log = ""
            for _sib_id, _sib_name, _sib_user in _siblings:
                _ccd = f"/etc/openvpn/server/ccd/{_sib_user}"
                if not _os.path.exists(_ccd):
                    continue
                _cov = False
                for _ln in open(_ccd).read().splitlines():
                    _ln = _ln.strip()
                    if _ln.startswith("iroute"):
                        _ps = _ln.split()
                        if len(_ps) >= 3 and _myip is not None:
                            try:
                                _net = _ipa.ip_network(
                                    f"{_ps[1]}/{_ps[2]}", strict=False)
                                if _myip in _net:
                                    _cov = True; break
                            except Exception:
                                pass
                if not _cov:
                    continue
                _line = next((l for l in _log.splitlines()
                               if l.startswith("CLIENT_LIST," + _sib_user + ",")),
                              None)
                if _line:
                    _parts = _line.split(",")
                    return {"ok": True, "status": "connected_shared",
                            "shared_with_olt_id": _sib_id,
                            "shared_with_olt_name": _sib_name,
                            "shared_with_user": _sib_user,
                            "endpoint": _parts[2] if len(_parts) > 2 else "",
                            "tunnel_ip": _parts[3] if len(_parts) > 3 else "",
                            "since": _parts[7] if len(_parts) > 7 else "",
                            "message": (f"Reaching this OLT via shared tunnel "
                                         f"from sibling OLT \"{_sib_name}\" "
                                         f"(CN={_sib_user}). No separate cert "
                                         f"needed — both OLTs sit on the same "
                                         f"customer LAN.")}
        return {"ok": True, "status": "never_provisioned",
                "message": "No credentials issued yet. Click Provision VPN first."}

    if vpn_type == "openvpn":
        try:
            log = open("/var/log/openvpn/olt-status.log").read()
        except Exception as e:
            return {"ok": False, "status": "error",
                    "message": f"OpenVPN status log unreachable: {e}"}
        # Find row for this Common Name (cert CN == username)
        line = next((ln for ln in log.splitlines()
                     if ln.startswith("CLIENT_LIST," + user + ",")), None)
        if not line:
            return {"ok": True, "status": "never",
                    "message": ("Cert issued but no successful connection "
                                "from this OLT yet. Check Mikrotik OVPN "
                                "client interface is in `R` state.")}
        parts = line.split(",")
        # cols: CLIENT_LIST, cn, real_ip:port, virt_ip, virt_ip6, rx, tx,
        #       connected_since, connected_since_t, username, cid, peer_id,
        #       data_channel_cipher
        rx, tx = int(parts[5] or 0), int(parts[6] or 0)
        return {"ok": True, "status": "connected",
                ...
                "message": f"OpenVPN tunnel up since {parts[7]}."}

    if vpn_type in ("l2tp_ipsec", "pptp"):
        # ppp interface status — parse `ip addr` for ppp* interfaces.
        try:
            r = _sp.run("ip -o addr show", shell=True, capture_output=True,
                         text=True, timeout=3).stdout
        except Exception as e:
            return {"ok": False, "status": "error", "message": str(e)}
        ppps = [ln for ln in r.splitlines() if "ppp" in ln and "inet " in ln]
        # We don't have a direct user→ppp mapping; show active count.
        try:
            chap = open("/etc/ppp/chap-secrets").read()
            line_present = any(ln.lstrip().startswith(f'"{user}"') for ln in chap.splitlines())
        except Exception:
            line_present = False
        if not line_present:
            return {"ok": True, "status": "credentials_missing",
                    "message": "chap-secrets entry not found — re-run Provision VPN."}
        return {"ok": True,
                "status": ("connected" if ppps else "never"),
                "active_ppp_count": len(ppps),
                "message": (f"{len(ppps)} active PPP session(s) on the server. "
                            "Cross-reference with your Mikrotik's "
                            f"{'L2TP' if vpn_type=='l2tp_ipsec' else 'PPTP'} "
                            "client status to confirm this OLT is one of them.")}

    return {"ok": False, "status": "unknown_vpn_type",
            "vpn_type": vpn_type}
