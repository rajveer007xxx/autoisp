...
def translate_sql(sql: str) -> str:
    ...
    # _S58O_DT_PARAM_INTERVAL_  Parameterised interval form, e.g.
    #   `datetime('now', ?)` or `datetime('now', :mins)`.
    #   We rewrite to `(NOW() + (param)::interval)`.  Callers in
    #   the codebase all bind a value like '-7 days' / '-24 hours'
    #   which Postgres accepts as a SQL `interval` literal.
    import re as _re_s58o
    s = _re_s58o.sub(
        r"datetime\(\s*'now'\s*,\s*(\?|:\w+|%\([^)]+\)s|%s)\s*\)",
        lambda m: f"(NOW() + ({m.group(1)})::interval)",
        s, flags=_re_s58o.IGNORECASE,
    )
