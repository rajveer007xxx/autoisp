...
def generate_invoice_pdf_fibernet(invoice_data: dict, company_data: dict, customer_data: dict, previous_invoices: list = None) -> bytes:
    ...
    company_info_text = f"<b>{company_name}</b><br/>" \
                       ...
                       (f"GSTIN - {company_data.get('gst_number')}<br/>" if (show_gst and company_data.get('gst_number')) else "") + \
    ...
    buyer_info_text = f"<b>Buyer</b><br/>" \
                     ...
                     f"<b>BILLING ADDRESS:</b> {customer_data.get('address', '')}<br/>" \
                     f"<b>Company Name:</b> {customer_data.get('company_name', 'N/A')}<br/>" \
                     (f"<b>GSTIN -</b> {customer_data.get('customer_gst_no')}<br/>" if (show_gst and customer_data.get('customer_gst_no')) else "") + \
                     f"<b>State:</b> {customer_data.get('state', 'Madhya Pradesh')}<br/>" \
    ...
    try:
        ...
        display_end_dt = end_date_dt - timedelta(days=1)
        period_str = f"PERIOD {start_date_dt.strftime('%d/%m/%Y')} TO {display_end_dt.strftime('%d/%m/%Y')}"
    except:
